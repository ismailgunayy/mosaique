...
import { clearCanvas, drawText, getStartingPoints, sleep, steps } from "./utils";
...
export const ChaosTriangle = ({ isStarted }: IChaosTriangle) => {
	...
	useEffect(() => {
		if (canvasRef.current?.getContext("2d") && textCanvasRef.current?.getContext("2d")) {
			...
			if (ctx && textCtx && isStarted) {
				...
				const drawPoint = (times = 23) => {
					for (let i = 0; i < times; i++) {
						...
						const middle = [(vertex[0] + lastPoint[0]) / 2, (vertex[1] + lastPoint[1]) / 2];
